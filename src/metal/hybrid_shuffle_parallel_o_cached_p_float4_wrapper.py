...
def load_kernel_source():
    metal_path = os.path.join(
        ...
        "flash_attention_hybrid_shuffle_parallel_o_cached_p_float4.metal"
    )
    with open(metal_path, "r") as f:
        return f.read()

def metal_flash_attention_hybrid_shuffle_parallel_o_cached_p_float4(Q, K, V):
    B, H, N, d = Q.shape
    scale = float(d ** -0.5)
    ...
    kernel = mx.fast.metal_kernel(
        name="flash_attention_hybrid_shuffle_parallel_o_cached_p_float4_kernel",
